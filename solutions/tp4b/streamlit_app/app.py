"""UI Streamlit du TP 4 (modules Terraform - meme surface fonctionnelle que TP 3)."""
...
st.set_page_config(page_title="TP 4 - Terraform AWS Free Tier (modules)", layout="wide")
st.title("TP 4 - Terraform AWS Free Tier (S3 + DynamoDB + SQS via modules)")
...
data_bucket = next((b for b in buckets if "-tp4-data-" in b), None)
if data_bucket:
    st.success(f"Bucket TP 4 : `{data_bucket}`")
    ...
    st.warning("Aucun bucket TP 4 detecte.")

# -----------------------------------------------------------------------------
# DynamoDB
# -----------------------------------------------------------------------------
st.header("DynamoDB")
items_table = f"{PROJECT_PREFIX}-tp4-items"
...
# -----------------------------------------------------------------------------
st.header("SQS")
events_queue = f"{PROJECT_PREFIX}-tp4-events"
