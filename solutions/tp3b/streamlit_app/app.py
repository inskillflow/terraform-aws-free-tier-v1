"""UI Streamlit du TP 3 (ajoute SQS)."""
...
st.set_page_config(page_title="TP 3 - Terraform AWS Free Tier (SQS)", layout="wide")
st.title("TP 3 - Terraform AWS Free Tier (S3 + DynamoDB + SQS)")
...
data_bucket = next((b for b in buckets if "-tp3-data-" in b), None)
if data_bucket:
    st.success(f"Bucket TP 3 : `{data_bucket}`")
    ...
    st.warning("Aucun bucket TP 3 detecte.")

# -----------------------------------------------------------------------------
# DynamoDB
# -----------------------------------------------------------------------------
st.header("DynamoDB")
items_table = f"{PROJECT_PREFIX}-tp3-items"
...
# -----------------------------------------------------------------------------
st.header("SQS")
events_queue = f"{PROJECT_PREFIX}-tp3-events"
